  ...
  const Pagination = ({ pageState, onPrev, onNext }) => (
    <div className="flex flex-col sm:flex-row items-center justify-between gap-3 mt-4">
      <p className="text-sm text-gray-500">
        Page {pageState.page} of {pageState.total_pages} &middot; {pageState.total} total records
      </p>
      <div className="flex items-center gap-2">
        <button
          ...
          className="px-4 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 disabled:opacity-40 hover:bg-gray-50 transition-colors"
        >
          Previous
        </button>
        <button
    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      <div className="bg-white rounded-2xl p-6 text-center shadow-sm border border-gray-100 hover:shadow-md transition-all">
        <div className="w-14 h-14 bg-brand-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
          <svg className="w-7 h-7 text-brand-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" /></svg>
        </div>
        <h3 className="font-bold text-navy-700 mb-2">Total Bookings</h3>
        <p className="text-4xl font-bold text-brand-600 font-heading">{counts.bookings}</p>
      </div>
      <div className="bg-white rounded-2xl p-6 text-center shadow-sm border border-gray-100 hover:shadow-md transition-all">
        <div className="w-14 h-14 bg-brand-100 rounded-2xl flex items-center justify-center mx-auto mb-4">
        ...
        <h3 className="font-bold text-navy-700 mb-2">Contact Messages</h3>
        <p className="text-4xl font-bold text-brand-600 font-heading">{counts.messages}</p>
      </div>
    </div>
  )
  ...
  const renderBookingActions = (booking) => {
    return (
      <div className="flex flex-wrap justify-end gap-2">
        <button type="button" onClick={() => updateBookingStatus(booking.id, 'approved')} className="px-3 py-2 rounded-xl bg-brand-500 text-navy-900 text-xs font-semibold hover:bg-brand-600 transition-colors">
          Approve
        </button>
        <button type="button" onClick={() => updateBookingStatus(booking.id, 'completed')} className="px-3 py-2 rounded-xl bg-green-600 text-white text-xs font-semibold hover:bg-green-700 transition-colors">
          Complete
        </button>
        <button type="button" onClick={() => updateBookingStatus(booking.id, 'canceled')} className="px-3 py-2 rounded-xl bg-red-600 text-white text-xs font-semibold hover:bg-red-700 transition-colors">
          Cancel
        </button>
      </div>
    )
            ...
            {bookingData.items.map((booking) => (
              <tr key={booking.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-4">
                  <div className="font-semibold text-navy-700">{booking.full_name}</div>
                  <div className="text-sm text-gray-500">{booking.email}</div>
                </td>
                <td className="px-4 py-4 text-sm text-gray-600">{booking.service_type}</td>
                <td className="px-4 py-4">
                  <span className={`inline-flex px-3 py-1 rounded-full text-xs font-semibold ${statusBadge(booking.status)}`}>
                    {booking.status}
                  </span>
                </td>
                <td className="px-4 py-4 text-sm text-gray-500">{formatDate(booking.created_at)}</td>
                <td className="px-4 py-4">
                  <div className="flex flex-col items-end gap-2 sm:flex-row sm:flex-wrap">
                    <button type="button" onClick={() => openRecord('booking', booking)} className="px-3 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                      View
                    </button>
          ...
          <tbody className="divide-y divide-gray-100">
            {messageData.items.map((message) => (
              <tr key={message.id} className="hover:bg-gray-50 transition-colors">
                <td className="px-4 py-4">
                  <div className="font-semibold text-navy-700">{message.name}</div>
                  <div className="text-sm text-gray-500">{message.email}</div>
                </td>
                <td className="px-4 py-4 text-sm text-gray-600">{message.subject}</td>
                <td className="px-4 py-4">
                ...
                <td className="px-4 py-4 text-sm text-gray-500">{formatDate(message.created_at)}</td>
                <td className="px-4 py-4">
                  <div className="flex justify-end gap-2">
                    <button type="button" onClick={() => openRecord('message', message)} className="px-3 py-2 rounded-xl border border-gray-200 text-sm font-medium text-gray-700 hover:bg-gray-50 transition-colors">
                      View
                    </button>
                  </div>
                </td>
  ...
  return (
    <div className="min-h-screen bg-brand-50">
      <div className="md:hidden sticky top-0 z-40 bg-white/90 backdrop-blur border-b border-gray-200 px-4 py-3 flex items-center justify-between">
        <button type="button" onClick={() => setSidebarOpen(true)} className="px-3 py-2 rounded-xl border border-gray-200 text-gray-700 hover:bg-gray-50 transition-colors">
          Menu
        </button>
        <div className="font-bold text-navy-700 font-heading">E Liz LLC Admin</div>
      ...
      <div className="flex w-full max-w-full">
          ...
          <div className="p-6 h-full flex flex-col">
              ...
              <div>
                <div className="flex items-center gap-2 mb-1">
                  <div className="w-8 h-8 bg-navy-700 flex items-center justify-center">
                    <span className="text-brand-500 font-bold text-sm font-heading">E</span>
                  </div>
                  <h2 className="text-lg font-bold text-navy-700 font-heading">E Liz LLC</h2>
                </div>
              ...
              {[
                ...
                <button
                  ...
                  className={`w-full text-left px-4 py-3 rounded-xl transition-colors ${
                    activeTab === key ? 'bg-brand-100 text-brand-700 font-semibold' : 'text-gray-600 hover:bg-brand-50'
                  }`}
                >
                  {label}
                </button>
            ...
            <div className="mt-auto pt-6">
              <button
                ...
                className="w-full text-left px-4 py-3 rounded-xl text-red-600 hover:bg-red-50 font-semibold transition-colors"
              >
                Logout
              </button>
            </div>
        ...
        <main className="flex-1 min-h-screen p-4 md:p-8 md:ml-0 min-w-0 max-w-full">
          <div className="hidden md:flex items-center justify-between mb-6">
            <div>
              <h1 className="text-2xl font-bold text-navy-700 capitalize font-heading">{activeTab.replace('-', ' ')}</h1>
              <p className="text-gray-500">Manage bookings and messages.</p>
            </div>
          </div>
            ...
            {modalState.loading ? (
              ...
              <div className="max-h-[92vh] overflow-y-auto p-5 sm:p-6 md:p-8">
                <div className="flex items-start justify-between gap-4 mb-6">
                  <div>
                    <h3 className="text-2xl font-bold text-navy-700 font-heading">
                  ...
                  <button type="button" onClick={closeModal} className="text-gray-500 hover:text-gray-900 transition-colors">
                ...
                {modalState.type === 'booking' && modalState.data && (
                  <div className="space-y-4">
                    ...
                    <div className="flex flex-wrap gap-3 pt-2">
                      <button type="button" onClick={() => updateBookingStatus(modalState.data.id, 'approved')} className="px-4 py-3 rounded-xl bg-brand-500 text-navy-900 font-semibold hover:bg-brand-600 transition-colors">
                        Approve
                      </button>
                      <button type="button" onClick={() => updateBookingStatus(modalState.data.id, 'completed')} className="px-4 py-3 rounded-xl bg-green-600 text-white font-semibold hover:bg-green-700 transition-colors">
                        Complete
                      </button>
                      <button type="button" onClick={() => updateBookingStatus(modalState.data.id, 'canceled')} className="px-4 py-3 rounded-xl bg-red-600 text-white font-semibold hover:bg-red-700 transition-colors">
                        Cancel
                      </button>
                    </div>
                  </div>
          ...
          <div className="w-full max-w-md rounded-3xl bg-white shadow-2xl">
            <div className="p-6 sm:p-8">
              <div className="w-12 h-12 bg-red-100 rounded-2xl flex items-center justify-center mb-4">
                <svg className="w-6 h-6 text-red-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L3.34 16.5c-.77.833.192 2.5 1.732 2.5z" /></svg>
              </div>
              <h3 className="text-2xl font-bold text-navy-700 font-heading">Log out?</h3>
              ...
              <div className="mt-6 flex flex-col-reverse gap-3 sm:flex-row sm:justify-end">
                ...
                <button
                  ...
                  className="rounded-xl bg-red-600 px-4 py-3 text-sm font-semibold text-white hover:bg-red-700 transition-colors"
                >
                  Log out
                </button>
              </div>
...
function Detail({ label, value }) {
  return (
    <div className="rounded-2xl border border-gray-100 bg-gray-50 p-4">
      ...
      <div className="text-sm text-navy-700 whitespace-pre-wrap">{value}</div>
    </div>
  )
}
